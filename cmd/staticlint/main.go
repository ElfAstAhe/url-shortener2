// Package main encapsulate multichecker
package main
...
func main() {
	...
	// any U class analyzer (we need only one)
	for _, check := range staticcheck.Analyzers {
		if strings.HasPrefix(check.Analyzer.Name, "U") {
			analyzers = append(analyzers, check.Analyzer)
			break
		}
	}

	// full slice of analyzers by task iteration 20
